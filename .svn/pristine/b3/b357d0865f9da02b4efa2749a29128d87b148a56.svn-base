...
(function($){
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                var steps = $("ul > li > p > a", obj); // Get all anchors in this array
                var contentWidth = 0;
                var loader,msgBox,elmActionBar,elmStepContainer,btNext,btPrevious,btFinish,txTip;
                ...
                function init(){
                  ...
                  txTip=$('<label class="txTip">"*"为必填项</label>');
                  ...
                  elmActionBar.append(btFinish).append(btNext).append(btPrevious).append(txTip); 
                  contentWidth = elmStepContainer.width();

                  $(btNext).click(function() {
                	  if ($(this).hasClass("buttonDisabled")) return false; //add by [hgq]
                      doForwardProgress();
                      return false;
                  }); 
                  ...
                  if(options.keyNavigation){
                      $(document).keyup(function(e){
                          if(e.which==39){ // Right Arrow
                        	if ($(btNext).hasClass("buttonDisabled")) return false; //add by [hgq]
                            doForwardProgress();
                ...
                function showStep(stepIdx){
                    ...
                    if(stepIdx != curStepIdx){
                      if($.isFunction(options.onLeaveStep)) {
                        if(!options.onLeaveStep.call(this,$(curStep),stepIdx)){
                          return false;
                        }
                      }
                    }     
