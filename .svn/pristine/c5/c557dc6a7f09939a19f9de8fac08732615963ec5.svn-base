...
Vanadium.extend(
{
  ...
  getElementType: function(element) {
    switch (true) {
      ...
      case (element.nodeName.toUpperCase() == 'INPUT' && element.type.toUpperCase() == 'RADIO'):
          return Vanadium.RADIO;
  ...
  is_input_element : function(element) {
    return (element.nodeName.toUpperCase() == 'TEXTAREA') ||
           ...
           (element.nodeName.toUpperCase() == 'INPUT' && element.type.toUpperCase() == 'RADIO') ||
...
ElementValidation.prototype = {
  ...
  create_advice: function(validation_result) {
    ...
    jQuery(this.element).parent().append(span);
    return span;
  },
  reset: function() {
    this.invalid = undefined; //mark this validation element as undefined
  ...
  observe: function() {
    ...
    var self = this;
    jQuery(element).focus(function() { //此事件在<input type="file" 元素下，在chrome浏览器会有问题，不触发事件 
      self.virgin = false;
    });
    switch (elementType) {
      ...
      case Vanadium.FILE:
        jQuery(element).change(function() {
          self.virgin = false;//chrome下补触发focus事件，须在此加这个。
...
/*下面的 add by [hgq]*/
['CarNum',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^[a-zA-Z_0-9]{5}$/.test(v))
},'车牌号格式不正确'],
['mobile',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^0?(13|15|18)[0-9]{9}$/.test(v))
},'手机号码格式不正确'],
////电话号码的验证(包括验证国内区号,国际区号,分机号)
['tel',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^[0-9\-()（）]{7,18}$/.test(v))
},'电话号码格式不正确'],
['zipcode',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^[1-9][0-9]{5}$/.test(v))
},'邮编格式不正确'],
['picture',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /\.(jpg|jpeg|bmp|gif|png)$/.test(v))
},'只能上传(jpg、jpeg、bmp、gif)格式的图片文件'],
['word',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^.*?\.(doc|docx)$/.test(v))
},'只能上传Microsoft Office Word文档文件'],
['docNum',function(v){
	   return (Vanadium.validators_types['empty'].test(v) || /^\S{12,12}$/.test(v))
	},'档案编号应该为12位数字编码'],
['cardNum',function(v){
	  return (Vanadium.validators_types['empty'].test(v) || /^[粤]{1}[A]{1}[a-zA-Z_0-9]{5}$/.test(v))
},'车牌号格式不正确'],	
['sixNum',function(v){
	 return (Vanadium.validators_types['empty'].test(v) || /^\S{6,6}$/.test(v))
},'只能是六位'],
['fourNum',function(v){
	 return (Vanadium.validators_types['empty'].test(v) || /^\S{4,4}$/.test(v))
},'只能是四位'],
['idcard',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^[1-9]([0-9]{14}|[0-9|X]{17})$/.test(v))
},'身份证号码格式不正确'],
['carcard',function(v){
	return (Vanadium.validators_types['empty'].test(v) || /^[1-9]([0-9]{14}|[0-9|X]{17})$/.test(v))
},'驾驶证号码格式不正确'],
/*下面的 add by [lam]*/
['allCarNum',function(v){
    return (Vanadium.validators_types['empty'].test(v) || /^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{5}$/.test(v))
},'车辆牌号格式不正确']
])
